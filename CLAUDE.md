...
**Show the queue, not the file names.** A merge tool concatenates files the user ordered by hand, and the mistake it invites — the wrong order, or a portrait clip among landscape ones — only surfaces after minutes of encoding. `useClipMetadata` (`lib/hooks/use-clip-metadata.ts`) measures each queued file with an offscreen media element, and `lib/tools/clip-summary.ts` turns the measurements into a running time and a size-mismatch check. Results are keyed by file identity, not by index, so probes that land while the user is reordering cannot attach to the wrong row. An unmeasured clip stays `null` and suppresses the total rather than showing one that keeps jumping.

**Verifying media tools:** `tests/fixtures/{videos/sample-1s.mp4,audio/sample-1s.mp3}` are hand-built minimums that browsers will not fully decode, so no `<video>`/`<audio>` ever reports a duration for them — fine for tests that only need a file, useless for anything touching metadata. Use `playable-4s.mp4`, `portrait-2s.mp4` (for size-mismatch cases) or `playable-3s.mp3` instead. Note that the mpeg4 encode in ffmpeg.wasm **hangs under Playwright's bundled Chromium** — headed or headless, and for the plain `compressVideo` path too, not just the estimate — so `/video/compress` cannot be covered end-to-end there. It works in a real browser. Cover that tool with unit and component tests instead.
...
- **Say why it failed, when you can tell.** Every media tool used to report one dead-end string for every cause. `mediaErrorMessage(err, messages.error, messages.mediaError)` (`lib/media-errors.ts`) picks out the two causes that are reliably identifiable — a lost cross-origin isolation (reload the page; the file is fine) and heap exhaustion (shorter clip or lighter preset) — and falls back to the tool's own wording otherwise. Anchor any new pattern on word boundaries: an unanchored `oom` matched "boom" and would have blamed the file for an unrelated failure.
...
**Drop zones: `onDragLeave={(e) => { if (leftDropZone(e)) setDragging(false); }}`.** `dragleave` fires whenever the pointer crosses into a child, so wiring it straight to `setDragging(false)` makes the accent border blink the whole way across a zone that contains a preview, a filename and a button. `leftDropZone` (`lib/drag-utils.ts`) checks `relatedTarget` against the zone. All 40 tools use it.
...
- **Never open a PDF twice.** `getPdfDocument(file)` (`lib/pdf-document-cache.ts`) keeps one parsed `PDFDocumentProxy` per `File` in a `WeakMap`, and `PdfThumbnail` goes through it. Each thumbnail used to run `new Uint8Array(await file.arrayBuffer())` + `getDocument()` on its own, so a 120-page document meant 120 copies of the file and 120 concurrent pdf.js documents. `PdfThumbnail` also waits for an `IntersectionObserver` hit before rendering, so a long document does not render every page at once.
- **Import pdf.js through `loadPdfjs()`** (`lib/pdfjs-loader.ts`), never with a bare `import('pdfjs-dist')`. It configures the self-hosted worker, and it is the only seam a test can replace: the real module touches `DOMMatrix` at module scope and cannot be imported in Node or jsdom at all.
- `usePdfPageCount` deliberately stays on pdf-lib rather than the shared pdf.js document: pdf.js needs a worker and a real canvas, which would make the hook and the five tools using it untestable in jsdom. One extra parse, against the N it saves on thumbnails.
