...
5. **Registry entry** in `lib/tools/registry.ts` — `{ id, category, href, i18nKey, keywords, acceptedMime, status }`. The hub (`app/[locale]/page.tsx`) renders cards from this list grouped by category; `lib/tools/mime-router.ts` uses it for drop-routing decisions; `app/sitemap.ts` auto-emits the route in both locales (no manual sitemap edit needed).
