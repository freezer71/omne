...
Every tool has 4 files and one registry entry. They are deliberately separated so logic is testable in pure Node and UI is testable in jsdom.
...
3. **Route page** at `app/[locale]/<category>/<id>/page.tsx` — server component that awaits `params`, calls `getDictionary(locale)`, calls `getToolMetadata(...)` from `lib/tools/metadata.ts` for `generateMetadata`, wraps the tool component in `<ToolShell>`, and pipes localized strings down as props.
4. **Registry entry** in `lib/tools/registry.ts` — `{ id, category, href, i18nKey, keywords, acceptedMime, status }`. The hub (`app/[locale]/page.tsx`) renders cards from this list grouped by category; `lib/tools/mime-router.ts` uses it for drop-routing decisions.

Add matching keys to **both** `messages/en.json` and `messages/fr.json` under `tools.<category>.<id>` — `tests/setup/i18n-keys.test.ts` enforces structural parity between the two dictionaries and will fail CI if a key is missing on either side.
