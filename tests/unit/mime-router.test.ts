...
describe('routeDroppedFiles', () => {
  ...
  it('routes a single .json file directly to /json/format', () => {
    const result = routeDroppedFiles([json()]);
    expect(result.kind).toBe('direct');
    if (result.kind !== 'direct') return;
    expect(result.tool.id).toBe('format');
    expect(result.tool.category).toBe('json');
  });
});
